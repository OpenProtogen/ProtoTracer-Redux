...
float SimplexNoise<colors>::Noise(float xin, float yin, float zin) {
	...
	if (t1 < 0)
		...
		t1 *= t1;
		n1 = t1 * t1 * grad3[gi1].DotProduct(Vector3D(x1, y1, z1));
		;  // dot(grad3[gi1], x1, y1, z1);
	}
...
template <size_t colors>
float SimplexNoise<colors>::FFloor(float f) {
	int fi = (int)f;
	return f < fi ? fi - 1 : fi;
}

template <size_t colors>
float SimplexNoise<colors>::FMin(float a, float b) { return a < b ? a : b; }

template <size_t colors>
float SimplexNoise<colors>::FMax(float a, float b) { return a > b ? a : b; }

template <size_t colors>
float SimplexNoise<colors>::FAbs(float f) { return f < 0 ? -f : f; }

template <size_t colors>
float SimplexNoise<colors>::FSqrt(float f) { return sqrtf(f); }

template <size_t colors>
float SimplexNoise<colors>::InterpHermite(float t) { return t * t * (3 - 2 * t); }

template <size_t colors>
float SimplexNoise<colors>::InterpQuintic(float t) { return t * t * t * (t * (t * 6 - 15) + 10); }

template <size_t colors>
float SimplexNoise<colors>::Lerp(float a, float b, float t) { return a + t * (b - a); }

template <size_t colors>
float SimplexNoise<colors>::CubicLerp(float a, float b, float c, float d, float t) {
	float p = (d - c) - (a - b);
	return t * t * t * p + t * t * ((a - b) - p) + t * (c - a) + b;
}

template <size_t colors>
float SimplexNoise<colors>::PingPong(float t) {
	t -= (int)(t * 0.5f) * 2;
	return t < 1 ? t : 2 - t;
}

template class SimplexNoise<4>;
template class SimplexNoise<6>;