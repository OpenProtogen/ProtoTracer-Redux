...
bool Triangle2D::DidIntersect(BoundingBox2D& bbox) {
	...
	if (!(bbox.GetMinimum().X < tmax(p1X, p2X, p3X) && bbox.GetMaximum().X > tmin(p1X, p2X, p3X))) {
		return false;
	}

	if (!(bbox.GetMinimum().Y < tmax(p1Y, p2Y, p3Y) && bbox.GetMaximum().Y > tmin(p1Y, p2Y, p3Y))) {
		return false;
	}

	Vector2D axes[] = {{-1.0f * (p2Y - p1Y), p2X - p1X},
							 {-1.0f * (p3Y - p1Y), p3X - p1X},
							 {-1.0f * (p3Y - p2Y), (p3X - p2X)}};  // axes for SAT-based intersection testing
	...
	if (max(-1.0f * max(p0, p2), min(p0, p2)) > r) return false;
	...
	if (max(-1.0f * max(p0, p1), min(p0, p1)) > r) return false;
